...
const STORAGE_KEY = "gruzgame06:players";
...
export default function Home() {
  ...
  const name = useMemo(
    () => context?.user?.displayName || (address ? `Игрок ${address.slice(2, 6).toUpperCase()}` : "Игрок"),
  ...
  const handleConnectWallet = async (connector = preferredConnector) => {
    if (!connector) {
      setError("Установи Rabby или MetaMask и попробуй подключить кошелёк снова.");
      return;
    }
  ...
  const handleSyncTaps = async () => {
    ...
    try {
      ...
      const data = withGruzGame06BuilderCodeDataSuffix(
        encodeFunctionData({
          abi: gruzGame06OnchainAbi,
          functionName: "tap",
      ...
      <section className={styles.card}>
        ...
        {!isConnected || !address ? (
          <div className={styles.walletPanel}>
            <p className={styles.warning}>Подключи Rabby, MetaMask или Base Account, чтобы играть.</p>
            <button
        ...
        {view === "tap" && (
          <div className={styles.viewBlock}>
            <button
              className={styles.tyankaButton}
              type="button"
              onClick={handleTap}
              disabled={!state || isBusy || !isCorrectChain}
              style={{ transform: tapPulse % 2 === 1 ? "scale(0.98)" : undefined }}
            >
              <Image
                className={styles.tyankaArt}
                src="/anime-hero.svg"
                alt="Аниме тянка"
                width={280}
                height={280}
                priority
              />
              <span className={styles.tyankaCaption}>ТАПАЙ ТЯНКУ ✨</span>
            </button>
